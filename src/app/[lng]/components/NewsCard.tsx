...
export const NewsCard = ({ article, onClick }: NewsCardProps) => {
  ...
  const getVocabularyComplexity = () => {
    ...
    const avgWordLength =
      article.content.split(" ").reduce((acc, word) => acc + word.length, 0) /
      wordCount;

    if (avgWordLength > 6)
      return { level: "Rich", icon: Star, color: "text-shiba-navy" };
    if (avgWordLength > 4.5)
      return { level: "Moderate", icon: Zap, color: "text-shiba-orange" };
  ...
  return (
    <Card
      className={`cursor-pointer hover:shadow-xl hover:shadow-shiba-teal/20 transition-all duration-300 hover:-translate-y-1 group bg-gradient-to-br ${levelInfo.gradient} border-2 hover:border-shiba-teal/40 relative overflow-hidden rounded-2xl`}
      onClick={onClick}
    >
      ...
      <div className="absolute top-3 right-3 z-10">
        <div
          className={`rounded-full p-2 ${levelInfo.color} border-2 shadow-sm backdrop-blur-sm bg-white/80`}
        >
      ...
      <CardHeader className="pb-3 relative">
        ...
        */}
        ...
        <h3 className="font-semibold line-clamp-2 text-shiba-navy group-hover:text-shiba-dark-teal transition-colors">
          {article.title}
        </h3>
      </CardHeader>

      <CardContent>
        <p className="text-shiba-navy/70 text-sm line-clamp-3 mb-4 group-hover:text-shiba-navy transition-colors">
          {article.summary}
        </p>
